...
### 4.1.2 Exploring firm-level and industry-level variables

#### 3.3.2.1 The firm's revenue in difference province
```{r}
...
#### 3.3.2.2 The influcene of firm's total employees
```{r, message=FALSE}
...
The p value is 0.1419, which is larger than 0.05, so we can not reject the Null Hypothesis on a 95 % confidence level. We can not find a significant difference in the two means.
...
ggplot(data = mnc, aes(x = lceopay, y = revenue)) + geom_point(alpha = 1/2) + theme_minimal() + geom_smooth(aes(col = ownership), method = 'lm')
